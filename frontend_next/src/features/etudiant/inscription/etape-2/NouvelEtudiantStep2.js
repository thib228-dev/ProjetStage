    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-150 via-yellow-50 to-blue-200 font-sans flex flex-col">
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12 pt-24">
        <div className="w-full max-w-lg mb-6 self-start px-2">
          <Link href="/etudiant/inscription/etape-1" className="text-black font-semibold hover:underline">
             ← page précédente
          </Link>
        </div>
        ...
        className="bg-transparent   px-8 py-10 w-full  shadow-xl max-w-lg flex flex-col gap-8 animate-fade-in border border-gray-300 ">
          {/* Upload photo */}
          <div className="flex flex-col items-center gap-2">
            ...
            <div className="relative w-32 h-32 rounded-full bg-gray-100 border-2 border-gray-300 flex items-center justify-center overflow-hidden mb-2">
              ...
              {/* input caché pour choisir une photo */}
              <input id="fileInput" type="file" accept="image/*" className="absolute inset-0 opacity-0 cursor-pointer" />
            
              {/* bouton modifier visible */}
              <label htmlFor="fileInput" className="absolute bottom-0 bg-white/90 text-black font-medium text-xs px-2 py-1 rounded-full shadow flex items-center gap-1 cursor-pointer">
